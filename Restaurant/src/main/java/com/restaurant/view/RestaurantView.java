...
// 最小尺寸
        ...
        javax.swing.Timer refreshTimer = new javax.swing.Timer(600000, e -> {
            if (controller != null) {
    ...
    private void showSimpleDineInCheckout(String tableNumber) {
        ...
        // 总金额标签
        JLabel totalLabel = new JLabel("总金额: 0.00 元");
        ...
        checkoutButton.addActionListener(e -> {
            try {
                ...
                if (confirm == JOptionPane.YES_OPTION) {
                    new Thread(() -> {
                        controller.handleCheckoutSubmit("DINE_IN", tableNumber, paymentAmount);
                        SwingUtilities.invokeLater(() -> {
                            dialog.dispose();
                        });
                    }).start();
...
// 🔧【关键】加载订单详情时更新金额显示
                ...
                if (checkConfig != null && checkConfig.isSelected()) {
                    ...
                    if (twoCheck != null && twoCheck.isSelected()) {
                        try {
                            ...
                            if (qty > 0) tableConfig.put("2", qty);
                    ...
                    if (fourCheck != null && fourCheck.isSelected()) {
                        try {
                            ...
                            if (qty > 0) tableConfig.put("4", qty);
                        } catch (NumberFormatException ignored) {}
                    }
                    ...
                    if (sixCheck != null && sixCheck.isSelected()) {
                        try {
                            ...
                            if (qty > 0) tableConfig.put("6", qty);
                        } catch (NumberFormatException ignored) {}
                    }
    ...
    private void rebuildFormPanel(JPanel formPanel, String mode,
        ...
        if ("CREATE".equals(mode)) {
            ...
            // ═══════════════════════════════════════════════════════════
            JPanel infoPanel = new JPanel(new GridLayout(5, 2, 10, 5));  // 🔧 修改：从 4 行改为 5 行
            ...
            if (controller != null) {
                try {
                    ...
                    if (allTables != null && !allTables.isEmpty()) {
                        ...
                        JLabel noTableLabel = new JLabel("⚠️ 暂无空闲餐桌", SwingConstants.CENTER);
                ...
                JLabel initLabel = new JLabel("⚠️ 控制器未初始化", SwingConstants.CENTER);
                ...
                try {
                    ...
                    if (results.size() == 1) {
                        ...
                        fillReservationInfo(reservation, infoReservationId, infoName,
                                infoPhone, infoTime, tableConfigLabel);
        ...
        // ═══════════════════════════════════════════════════════════
            ...
            JRadioButton changeToYesRadio = new JRadioButton("改为：是（可预点餐 + 预付）");
    ...
    public String showReservedTableDialog(Tables table) {
        ...
        // 格式化显示预定时间
        String reservedTimeStr = "未设置";
        if (table.getReservedTime() != null) {
            reservedTimeStr = table.getReservedTime()
                    .format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"));
        }
        infoPanel.add(new JLabel("预定时间:"));
        infoPanel.add(new JLabel(reservedTimeStr));

        infoPanel.add(new JLabel("关联桌号:"));
        infoPanel.add(new JLabel(table.getGroupWith() != null ? table.getGroupWith() : "无"));
        ...
        // 按钮3：📄 查看详情（占位）
        JButton delayBtn = new JButton("延遲");
        delayBtn.setFont(new Font("Microsoft YaHei", Font.PLAIN, 13));
    ...
    public void refreshQuantityReservationsLog() {
        if (controller == null) {
            System.out.println("⚠️ controller未初始化，跳过预约列表刷新");
            return;
        }
            ...
            SwingUtilities.invokeLater(() -> {
                ...
                if (reservations.isEmpty()) {
                    ...
                    for (Map<String, Object> res : reservations) {
                        ...
                        // 🔧 三色状态判断逻辑
                        enum ReservationStatus { EXPIRED, URGENT, NORMAL }
                        ...
                        JPanel reservationPanel = createReservationPanelSeparated(
                                resId, resTime, formatter, status,
                                bgColor, borderColor, circleColor, textColor, circleSymbol
                        );

                        // 添加到主面板
                        reservationsLogPanel.add(reservationPanel);
                        reservationsLogPanel.add(Box.createRigidArea(new Dimension(0, 6)));  // 记录间距
                    }
                }
    ...
    private JPanel createReservationPanelSeparated(
            String resId, LocalDateTime resTime, DateTimeFormatter formatter,
        ...
        // 第二行：时间 + 状态标签
        if (resTime != null) {
            String timeStr = resTime.format(formatter);
            String statusText = (status.toString().equals("EXPIRED")) ? " (已過期)" : "";
            JLabel timeLabel = new JLabel(timeStr + statusText);
            timeLabel.setForeground(status.toString().equals("EXPIRED") ?
                    new Color(33, 150, 243) : new Color(120, 120, 120));
            timeLabel.setFont(new Font("Microsoft YaHei", Font.PLAIN, 11));
    ...
    private void showReservationDetailDialog(TableReservation reservation) {
        ...
        String timeStr = reservation.getReservationTime()
                .format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"));
        infoPanel.add(new JLabel(timeStr));
            ...
            if (result != null && "ASSIGN".equals(result.get("mode"))) {
                try {
                    ...
                    if (controller != null && resId != null && selectedTables != null && !selectedTables.isEmpty()) {
                        // 🔧 调用 Controller 分配餐桌
                        controller.handleReserveTable(new java.awt.event.ActionEvent(this, 0, "ASSIGN"));

                        // 🔧 显示成功提示
                        JOptionPane.showMessageDialog(dialog,
                                "✅ 餐桌分配成功！\n预约号：" + resId + "\n分配餐桌：" + String.join(", ", selectedTables),
                                "成功",
                                JOptionPane.INFORMATION_MESSAGE);
                    }
                } catch (Exception ex) {
                    JOptionPane.showMessageDialog(dialog,
                            "❌ 分配餐桌失败：" + ex.getMessage(),
                            "错误",
                            JOptionPane.ERROR_MESSAGE);
                    ex.printStackTrace();
                }
